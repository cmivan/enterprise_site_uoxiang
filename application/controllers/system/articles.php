...
class Articles extends HT_Controller {
	...
	function __construct()
	{
		...
		$this->data['typeB'] = $this->Articles_Model->types_box();
	}
...
/* ********** 管理 *********** */
	function manage()
	{
		...
		switch($cmd)
		{
			...
			case 'move':
				  if( is_array($id) )
				  {
					  ...
					  $T = $this->Articles_Model->get_type_arr( $type_id );
					  $this->db->where_in('id',$id);
		...
		if( $typeB_id )
		{
			$this->data['typeS'] = $this->Articles_Model->types_box( $typeB_id );
		}
...
/* ********** 添加/编辑 *********** */
	...
	function edit_save()
	{
		...
		$data['content'] = auto_save_image($content);
...
/* ********** 分类管理页面 *********** */
	...
	function type()
	{
		...
		if($go=='yes')
		{
			...
			$row = $this->Articles_Model->get_type( $type_id );
			if(!empty($row))
			{
				//执行重新排序
