...
class Columns extends HT_Controller {
	...
	function __construct()
	{
		...
		$this->data['typeB'] = $this->Columns_Model->types_box();
	}
...
/* ********** 管理 *********** */
	function manage()
	{
		...
		if( $typeB_id )
		{
			$this->data['typeS'] = $this->Columns_Model->types_box( $typeB_id );
		}
...
/* ********** 分类管理页面 *********** */
	...
	function type()
	{
		...
		if( $del_id )
		{
			$this->Columns_Model->del_type($del_id);
			//重新获取分类
			$this->data['typeB'] = $this->Columns_Model->types_box();
		}
		...
		if($go=='yes')
		{
			...
			$row = $this->Columns_Model->get_type( $type_id );
			if(!empty($row))
			{
				//执行重新排序
	...
	function type_edit()
	{
		...
		if( $id )
		{
			...
			$rs = $this->Columns_Model->get_type($id);
			if(!empty($rs))
			{
	...
	{
		$re_row = $this->Columns_Model->get_types();
		if(!empty($re_row))
		{
			$re_num = $this->Columns_Model->get_types_num();
