...
class Kindeditor {
    ...
    private $ext_arr;
    private $max_size;
	private $is_js_load = false;
 
    public function __construct() {
		$this->base_path = '/public/up/k/';
	...
	{
		if( $this->is_js_load==false )
		{
		$this->is_js_load = true;
		return '<script charset="utf-8" type="text/javascript" src="'.base_url().'public/plugins/kindeditor/kindeditor.js"></script>';
		}
	}
	
	
    public function up_img($boxID='pic_s',$btuID='pic_s_btu',$imgW='',$imgH='')
	{
		$imgW = get_num($imgW);
		$imgH = get_num($imgH);
		...
		$edit.= "var uploadbutton = K.uploadbutton({";
		$edit.= "button : K('#".$btuID."')[0],";
		$edit.= "fieldName : 'imgFile',";
		$edit.= "url : '".site_system('editor/upload')."?mark=1&w=".$imgW."&h=".$imgH."',";
		$edit.= "afterUpload : function(data) {";
		$edit.= "if (data.error === 0) {";
		$edit.= "var url = K.formatUrl(data.url, 'absolute');";
		$edit.= "K('#".$boxID."').val(url);";
		$edit.= "} else { alert(data.message); }},";
		$edit.= "afterError : function(str) { alert('错误信息: ' + str); }});";
		$edit.= "uploadbutton.fileBox.change(function(e) {";
		$edit.= "uploadbutton.submit();";
		$edit.= "});});";
		$edit.= "</script>";
		return $edit;
	}
    ...
    public function up_file($boxID='pic_s',$btuID='pic_s_btu')
	{
		$edit = $this->js_load();
		$edit.= "<script>";
		$edit.= "KindEditor.ready(function(K) {";
		...
		$edit.= "button : K('#".$btuID."')[0],";
		$edit.= "fieldName : 'imgFile',";
		$edit.= "url : '".site_system('editor/upload')."?mark=1&dir=file',";
		$edit.= "afterUpload : function(data) {";
		$edit.= "if (data.error === 0) {";
		$edit.= "var url = K.formatUrl(data.url, 'absolute');";
		$edit.= "K('#".$boxID."').val(url);";
    ...
    public function js($boxID='content',$content='',$width='90%',$height='300px')
	{
		...
		$edit.= 'uploadJson : "'.site_system('editor/upload').'",'; //更改图片上传
		$edit.= 'fileManagerJson : "'.site_system('editor/manage').'",'; //更改图片浏览
    ...
    public function js_full($boxID='content',$content='',$width='90%',$height='300px')
	{
		$edit = $this->js_load();
		$edit.= "<script>";
		$edit.= "var editor;";
    ...
    public function upload($dir='',$file,$logid=0,$mark=0,$w='',$h='')
	{
		$mark = get_num($mark,0);
		$w = get_num($w,700);
		$h = get_num($h,700);
        ...
        if (empty($file) === FALSE) {
            ...
            if (@is_dir($this->root_path) === false) {
                $this->alert("上传目录:".$this->root_path." 不存在。".$this->root_path);
            }
            //检查目录写权限
			...
			if($dir_name=='image')
			{
				...
				{
					$CI =& get_instance();
					$CI->load->library('watermark');
					$CI->watermark->Mark($tmp_name);
				}
			}
