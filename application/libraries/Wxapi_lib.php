...
class Wxapi_lib {
   ...
   //最新
    ...
    //京东无节操
	private function JD_Unchaste()
	{
		$arr[]=array("《8种阴*刺激技巧》","","https://mp.weixin.qq.com/cgi-bin/getimgdata?mode=large&source=file&fileId=10013118%3E&token=1189974311&lang=zh_CN","http://sale.jd.com/act/PpEUOfiVtXq.html");
		$arr[]=array("高潮公开课第三期！（第二期已经被第三期覆盖了，所以没第二期的的链接了）","","https://mp.weixin.qq.com/cgi-bin/getimgdata?mode=large&source=file&fileId=10013120%3E&token=1189974311&lang=zh_CN","http://sale.jd.com/act/2cLhlYHFf4bUiMOz.html");
		$this->response_news($arr,array(2,0));
	}

    //互粉
	private function hufen($FromUserName='')
	{
		$this->response_text("/::)很感谢你对优享的支持，你的互粉信息已经收到并通知客服，请耐心等待客服处理哦！");
	}
	
    //newyear1314
	private function newyear1314()
	{
		$arr[]=array("感谢你的陪伴，有你一起走过1314！","","http://1314.uoxiang.com/images/view.jpg","http://1314.uoxiang.com");
		$this->response_news($arr,array(1,0));
	}
	...
	public function requestMsg()
	{
		...
		if(!empty($postStr)){
			...
			$this->MsgId = empty($postObj->MsgId)?'':$postObj->MsgId;
			return $postObj;
		}else{
	...
	//6 关注或取消事件处理
    public function request_event($msg='')
	{
		if($msg->Event=='subscribe')
		{
			//$this->response_text($msg->FromUserName.',欢迎关注！');
			//$this->response_text('关注优享生活范,让生活更有范！');
			$this->newyear1314();
		}elseif($msg->Event=='unsubscribe'){
			$this->response_text($msg->FromUserName.',我们会改进做好的,欢迎再次关注优享生活范！');
		}
	}
