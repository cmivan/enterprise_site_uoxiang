...
<td colspan="2" align="center">
  <a href="<?php echo site_system($dbtable.'/style_edit')?>">
  <img src="<?php echo site_url_fix($style['css_url'].'system_theme/ico/tree_explode','gif');?>" alt="添加风格" border="0" align="absmiddle" />
    添加 <span class="red"><?php echo $dbtitle;?></span> 风格
  </a></td>
<td width="40" align="center">排序</td>
<td width="120" align="center" class="edit_box_edit_td">操作</td></tr>
<?php
if(!empty($styleB)){
	foreach($styleB as $rs){
?>
<tr class="forumRow">
...
<td align="center"><input type="button" class="btu_del btu" url='<?php echo reUrl('del_id='.$rs['type_id'])?>' title='<?php echo $rs['type_title'];?>' tip='确定删除该大类？' value="&nbsp;" />  <input type="button" class="btu_edit btu" url='<?php echo site_system($dbtable.'/style_edit')?><?php echo reUrl('id='.$rs['type_id'])?>' value="&nbsp;"/>
</td>
</tr>
...
<input type="button" class="btu_edit btu" url='<?php echo site_system($dbtable.'/style_edit')?><?php echo reUrl('id='.$rsS['type_id'])?>' value="&nbsp;"/>
</td>
</tr>
<?php
	}
