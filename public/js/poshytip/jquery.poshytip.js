...
(function($) {
	...
	$.Poshytip.prototype = {
		init: function() {
			...
			// add by cmivan for user page fullcalendar
			var htmls = this.$elm.html();
			if(title==""||title==null){title=htmls;}
...
$(function(){
	//绑定样式
	$('#poshytip').attr('href',js_url+'poshytip/'+classname+'/'+classname+'.css');
	bindtip(); });
function bindtip(){
	$('.tip').poshytip({
		className: classname,
