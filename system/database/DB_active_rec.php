...
class CI_DB_active_record extends CI_DB_driver {
	...
	*/
	...
	function where_or_on($values = NULL)
	{
	...
	function like_on($values = NULL)
	{
		return $this->_like_on($values);
	}
	
	function like_or_on($values = NULL)
	{
		return $this->_like_on($values,'OR ');
	}
	
	function _like_on($values, $side = 'both', $type = 'AND ')
	{
		if ($values === NULL)
		{
			return;
		...
		foreach ($values as $value)
		{
			...
			if (is_array($value))
			{
				$this->ar_whereOR = $this->_like_on_set($value, $side);
			}
	...
	function _like_on_set($key, $side = 'both', $not = '', $escape = TRUE)
	{
		$key = $this->_object_to_array($key);
